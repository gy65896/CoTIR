...
try:
    ...
    _ver = getattr(_hf_transformers, "__version__", "?")
    raise ImportError(
        "CoTIR 的 Mistral-Small 文本塔需要 transformers>=4.51（提供 "
        "Mistral3ForConditionalGeneration）。"
        f"当前版本: {_ver}。请执行: pip install -U 'transformers>=4.51.0'"
    ) from e
...
def load_qwen3_embedder(variant: str, device: str | torch.device = "cuda"):
    # Prefer FP8 checkpoint when available, but automatically fall back on GPUs that
    # don't support fine-grained FP8 quantization (e.g. compute capability < 8.9).
    fp8_spec = f"Qwen/Qwen3-{variant}-FP8"
    bf16_spec = f"Qwen/Qwen3-{variant}"
    try:
        return Qwen3Embedder(model_spec=fp8_spec, device=device)
    except ValueError as e:
        msg = str(e)
        if "FP8" in msg and "compute capability" in msg:
            print(f"[warn] FP8 not supported on this GPU, falling back to {bf16_spec}")
            return Qwen3Embedder(model_spec=bf16_spec, device=device)
        raise


class Flux2Qwen3SubfolderEmbedder(nn.Module):
    """
    Use the FLUX.2 repo's bundled tokenizer + text_encoder subfolders.
    This matches the loading style in debug.py and avoids FP8-only Qwen checkpoints.
    """

    def __init__(
        self,
        flux2_model_dir_or_repo: str,
        device: str | torch.device = "cuda",
        ...
        if load_weights:
            ...
            self.model = Qwen3ForCausalLM(cfg)
